...
##      返戻分
        if  [ ${19} -eq '0' ] || [ ${19} -eq '2' ]; then
            ...
            $DBSTUB -dir $LDDIRECTORY -bd $PREFNAME $PROGRAMID -parameter $1,$2,$3,$RENNUM,$5,$6,$7,$8,$9,${10},${11},${12},${13},${15},${18},2,${20},${21},${16} > ${LOG_FILE}-2.log 2>&1
            if  [ -e ${MCP_TEMPDIR}/${16} ]; then
                exit
            fi
        fi
##      当月・月遅れ分
        if  [ ${19} -eq '0' ] || [ ${19} -eq '1' ]; then
            RENNUM=$(expr ${RENNUM} + 1) 
            $DBSTUB -dir $LDDIRECTORY -bd $PREFNAME $PROGRAMID -parameter $1,$2,$3,$RENNUM,$5,$6,$7,$8,$9,${10},${11},${12},${13},${15},${18},1,${20},${21},${16} > ${LOG_FILE}-1.log 2>&1
            if  [ -e ${MCP_TEMPDIR}/${16} ]; then
                exit
            fi
        fi
